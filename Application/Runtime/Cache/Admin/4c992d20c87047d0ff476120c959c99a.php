            ...
            <div class="rowbox">
               <div class="col-xs-12">
                    <form action="" class="form-inline">
                        ...
                        <input type="text" name="account" id="account" placeholder="输入要搜索的用户账号" value="<?php echo ($map["account"]); ?>" class="form-control">
                        <label for="">用户状态:</label>
                        <select name="status" id="status" class="select-input form-control">
                            <option value="" >全部</option>
                            <option value="1" <?php if(($map["state"]) == "1"): ?>selected<?php endif; ?>>正常</option>
                            <option value="0" <?php if(($map["state"]) == "0"): ?>selected<?php endif; ?>>冻结中</option>
                        </select>  
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php if(is_array($userlist)): $i = 0; $__LIST__ = $userlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td><?php echo ($vo["u_provice"]); ?></td>
                                    <td><?php echo ($vo["u_city"]); ?></td>
                                    <!-- <td><?php echo ($vo["u_state"]); ?></td> -->
                                    <td>
                                        <?php if(($vo["u_state"]) == "1"): ?>账户正常使用<?php endif; ?>
                                        <?php if(($vo["u_state"]) == "0"): ?><span style="color:red">账户冻结中 ...</s><?php endif; ?>
                                    </td>
                                    <td>
                                        <button id="userinfobtn" class="btn btn-primary" onclick="send('<?php echo ($vo["id"]); ?>')">发送通知</button>
                                        
                                        <button id="accfreebtn" class="btn btn-primary" onclick="dongjie('<?php echo ($vo["id"]); ?>')" <?php if(($vo["u_state"]) == "0"): ?>disabled<?php endif; ?>>账号冻结</button>
                                    </td>
...
<script>
    //发送通知
    function send(uid){
        location.href = "/index.php/Admin/Notices/sendnotice?uid="+uid;
    }
    //账号冻结
    function dongjie(uid){
        layer.confirm('确认冻结该用户', {icon: 3, title: '提示'},function(index){
            location.href = "/index.php/Admin/Users/dongjie?id="+uid;
            layer.close(index);
        })
    }
</script>
<script>
    ...
    $("#searchbtn").click(function(){
        ...
        console.log(status);
        location.href="/index.php/Admin/Users/userlist?account="+account+"&status="+status;
    })
</script>
</html>